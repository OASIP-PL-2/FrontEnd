...
<template>
  ...
  <main class="mx-10 -mt-4">
    ...
    <h2 class="mt-3 mb-5 text-4xl font-bold text-center text-black">{{ $route.params.time }} Events</h2>
    <!-- <p class="mb-5 text-xl font-semibold text-stone-600 col-10">The total of bookings are {{ filterEvent.length }}
      events
    </p> -->
    ...
    <div class="flex items-center justify-between mb-3 ml-6">
      <!-- filter time navbar -->
      <div class="relative flex px-4 text-center bg-gray-100 shadow-lg col-3 rounded-2xl"
    ...
    <div class="grid grid-cols-3 gap-6 mt-4 ml-12"> -->
      ...
      <router-link :to="{ name: 'AddNewEvent'}" >
        ...
        <div v-for="(event, index) in filterEvent" :key="index">
          <div class="max-w-md mx-auto overflow-hidden bg-white shadow-md rounded-xl hover:shadow-xl hover:scale-110">
            <div class="flex">
              <div class="w-4/12 lg:w-18 md:w-18 bg-gradient-to-r from-indigo-400 to-indigo-300">
                <p class="pt-6 text-3xl font-semibold text-center text-white">{{
                    extractDate(event.eventStartTime).getDate()
                }} </p>
                <p class="text-lg text-center text-white">{{ monthNames[extractDate(event.eventStartTime).getMonth()] }}
                </p>
                <p class="text-base text-center text-white">{{ extractDate(event.eventStartTime).getFullYear() }} </p>
              </div>
              <div class="m-3">
                <div class="text-lg font-semibold tracking-wide text-indigo-500 uppercase">{{ event.bookingName }}</div>
                <div class="block mt-1 text-base font-medium leading-tight text-black">{{
                    event.eventCategoryId.eventCategoryName
                }}</div>
                <p class="mt-1 text-slate-500">Start Time : {{ extractTime(event.eventStartTime) }}</p>

                <div class="mt-1 text-slate-500">{{ event.eventDuration }} min
      ...
      <span>
        <router-link :to="{ name: 'AddNewEvent' }">
          <div
            ...
            <p class="text-center text-white/50 group-hover:text-gray-900 group-hover:smooth-hover"> Booking Appointment
            </p>
          </div>
        </router-link>
      </span>
