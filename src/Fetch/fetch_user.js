...
const getUserDetail = async (id) => {
  ...
  if (res.status === 200) {
    ...
    Swal.fire(`Not found this user`, "Please Try again", "warning").then(
      (res) => {
    ...
    Swal.fire(`${response.message}`, "Please Try again", "warning").then(
      (res) => {
        window.location = document.referrer;
      }
    );
...
const editUserDetail = async (user, id) => {
  ...
  if (res.status === 200) {
    console.log("Successfully executed! " + res.status);
    Swal.fire("Edit Successfully", "You clicked the button!", "success").then(
      (res) => {
        window.location = document.referrer;
      }
    );
    ...
    const response = await res.json();
    Swal.fire(`${response.message}`, "Please Try again", "warning");
  } else {
