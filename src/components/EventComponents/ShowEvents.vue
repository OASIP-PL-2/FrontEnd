  ...
  <main class="mx-10 -mt-4">
    ...
    <div class="container" style="width: 80%;margin-bottom: 12px;">
      <div class="row">
        ...
        <div class="col-md-6 d-md-flex align-self-center justify-content-md-end justify-content-lg-end justify-content-xl-end justify-content-xxl-end" style="padding-right: 22px;padding-left: 7px;">
          <input class="form-control-sm" type="date" style="background: var(--bs-gray-100);border-radius: 50px;width: 40%;margin-top: auto;margin-left: 10px;color: #605e4f;font-size: 12px;border-width: 2px;border-style: solid;padding-right: 16px;padding-left: 16px;padding-top: 6px;padding-bottom: 6px;margin-bottom: 0px;" v-model="dateFilter" />
          <button class="btn btn-sm fw-semibold" type="button" style="padding-top: 0px;padding-bottom: 0px;color: #282821;font-size: 13px;border-radius: 16px;padding-right: 12px;padding-left: 12px;margin-left: 7px;background: #b5afa0;border: 2px solid #282821;" @click="filterByDate">
            Filter By Date
          </button>
        </div>
      </div>
    ...
    <div class="container" style="width: 80%;" v-if="filterEvent.length !== 0 ">
      ...
      <div class="row">
        <div v-for="event in filterEvent" :key="event.id" class="col-sm-12 col-md-6 col-lg-4" style="margin-bottom: 16px;">
            <div class="row" style="margin-right: -4px;">
              <div class="col-3 col-sm-3 col-md-3 col-lg-3 col-xl-3 col-xxl-3"
                style="background: #ffd451;box-shadow: 0px 5px 2px rgb(236,241,245);filter: blur(0px) brightness(100%);
                ...
                <div class="vstack text-center" style="margin-right: -1px;">
                  <!-- <h1>{{ event.id }} </h1> -->
                  <h4 class="fs-3" style="margin-bottom: 2px;margin-top: 15px;color: var(--bs-white);">{{ extractDate(event.eventStartTime).getDate() }}</h4>
                  <h6 class="fs-6" style="margin-bottom: 2px;color: var(--bs-white);font-size: 14px;">{{ monthNames[extractDate(event.eventStartTime).getMonth()] }}</h6>
                  <h6 style="margin-bottom: 7px;color: var(--bs-white);">{{ extractDate(event.eventStartTime).getFullYear() }}</h6>
                </div>
              </div>
              <div class="col-9 col-sm-9 col-md-9 col-lg-9 col-xl-9 col-xxl-9 text-left"
                style="box-shadow: 0px 5px 2px rgb(236,241,245);background: var(--bs-gray-100);border-top-right-radius: 15px;border-bottom-right-radius: 15px;
                ...
                <h6 class="text-uppercase fs-6 fw-semibold"
                  style="font-size: 20px;margin-top: 10px;color: var(--bs-yellow);margin-bottom: 2px;text-shadow: 2px 0px var(--bs-gray-200);
                  text-align: left;">
                  {{ event.bookingName }}
                </h6>
                ...
                <div class="d-sm-flex justify-content-sm-end">
                  <div class="col-12 col-sm-7 col-md-7 d-flex float-start">
                    <p style="color: var(--bs-gray-600);font-family: Assistant, sans-serif;font-size: 14px;">
                      {{ extractTime(event.eventStartTime)}} - {{ extractTime(event.eventStartTime , event.eventDuration)}}
                  ...
                  <div class="col-5 col-sm-5 col-md-5">
                    <div
                      ...
                      <i v-if="!(isLecturer)" class="fas fa-edit fs-6 text-white text-bg-primary border rounded-circle border-4 border-primary" style="margin-right: 3px;" @click="showEditForm(event)"></i>
                      <i v-if="!(isLecturer)" class="far fa-trash-alt fs-6 text-bg-danger border rounded-circle border-4 border-danger" @click="removeEvent(event)"></i>
                    </div>
                  </div>
                </div>
              </div>
    ...
    <!-- No event -->
    <div v-else v-show="isLogin">
      <span
        class="flex flex-col items-center px-4 py-20 mt-4 space-y-2 text-xl font-semibold text-center bg-orange-500 rounded-md cursor-pointer group">ไม่มีรายการนัดหมาย</span>
      <span>
        <router-link :to="{ name: 'Appointment' }">
          <div
            ...
            <p class="text-center text-white/50 group-hover:text-gray-900 group-hover:smooth-hover">
              Booking Appointment
            </p>
          </div>
        </router-link>
    ...
    <div class="container" style="width: 80%;" v-if="blindEvents.length !== 0 && !(isAdmin) && !(isLecturer)">
      <h6> Blind Events</h6>
      <div class="row">
        <div v-for="event in blindEvents" :key="event.id" class="col-sm-12 col-md-6 col-lg-4" style="margin-bottom: 16px;">
